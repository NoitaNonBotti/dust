...
## Example Django + React API connection

This repo now includes a small example vertical slice that connects the React
frontend to the Django backend for basic lost item records.
...
#### 6. What this tutorial proves

This confirms the basic full-stack flow:

```text
React form -> Django API -> SQLite database -> Django API -> React listing
```

This is still a starter example. The current connection only handles basic lost
item records and local development image storage. Full DUST features such as
account login, ownership claims, multiple categories, production media storage,
notifications, and admin permissions still need dedicated backend APIs.
